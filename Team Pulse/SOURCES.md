...
- [Это каркас, а не финальное ТЗ](#это-каркас-а-не-финальное-тз) — что решается один раз, а что заполняется потом
...
- [7. Паспорта метрик](#7-паспорта-метрик) — черновик по макету, 19 метрик таблицей
...
- [13. Открытые вопросы](#13-открытые-вопросы) — восемь штук, ни один не блокирует каркас
...
- [16. Роудмап](#16-роудмап) — этапы с критериями приёмки

---

## Это каркас, а не финальное ТЗ

Документ намеренно разделён на две части, и их не надо путать.

**Каркас — то, что решается один раз и переделывается дорого:**

- зёрна витрин (что чем является: запас, поток, суточный факт);
- правило числителя со знаменателем;
- разделение GP ↔ ClickHouse;
- материализованный путь и способ фильтрации поддерева;
- точки расширения — как добавляется метрика, разрез, категория, блок.

Всё это описано в §2–§6, §8 и §14, и оно **не зависит от того, чем окажется
конкретная метрика.** Текучесть можно считать по среднесписочной или по численности
на конец месяца, `regret` — месячным или накопительным, `hc_active` — с любым списком
статусов: каркас от этого не меняется, меняется SQL внутри одной витрины.

**Заполняется по мере проработки — то, что переделывается дёшево:**

- конкретные определения 19 метрик (§7);
- список систем-источников и их особенности;
- пороги, KPI, знаменатели, границы категорий (§13).

§7 и §13 — **черновик, снятый с макета**, а не согласованный контракт. Они здесь для
того, чтобы каркас было на чём проверить, и чтобы было видно, куда потом ложатся
ответы. Ни один из них не блокирует начало работ: витрины можно строить, пока
определения уточняются, потому что от определения зависит выражение внутри `SELECT`,
а не структура таблицы.

Правило простое: **если уточнение метрики требует поменять зерно витрины — значит,
зерно выбрано неверно, и это ошибка каркаса.** Пока таких случаев не нашлось.
...
История подразделений приходит готовой из источника — версионность в `d_unit`
переносится как есть, восстанавливать её не нужно. Отчёт читает срез дерева на
отчётный месяц, а `unit_path` в фактах фиксируется на тот же момент.
...
**Это черновик, снятый с макета, а не согласованный контракт.** Определения метрик ещё
в проработке; таблица здесь для того, чтобы каркас было на чём проверить, и чтобы было
видно, куда лягут будущие ответы.

Каркасом является **структура колонок**, а не значения в них: у каждой метрики обязаны
быть названы витрина, числитель, знаменатель и правило агрегации. Строка, у которой
знаменатель не назван, — это либо счётная метрика, либо незакрытый вопрос; третьего
не бывает.

Колонка «агрегация» — то, что делает ClickHouse после фильтрации.
...
Места, где макет договорился сам с собой, а прод так не сможет. **Ни один из них не
блокирует каркас** — все они меняют выражение внутри витрины, а не её зерно. Список
заполняется по мере проработки метрик; здесь он в том виде, в каком читается из макета.

Колонка «на что влияет» важнее колонки с решением: она показывает цену задержки. Если
влияние — «одна мера в одной витрине», вопрос спокойно живёт открытым до самого конца.

| # | Вопрос | На что влияет | Предлагаемое решение |
|---|---|---|---|
| 1 | **Что такое `hc_active`.** «Без длительных отсутствий» — сколько дней это «длительное»? Декрет, армия, больничный свыше N дней? | Одна мера в `f_person_month` | Список статусов согласовать с владельцем HR-ядра. Зерно витрины от ответа не зависит. |
| 2 | **Определение `regret`.** KPI 4 % / 7 % выглядит годовым, а `turnover_m` — месячная. Метрика месячная, накопительная или годовая? | Формула в ClickHouse | Считать накопительной с января, как `turnover_y`, — тогда KPI 4/7 читается. Слагаемые в витрине одни и те же при любом ответе. |
| 3 | **Знаменатель `low_perf` и `underwork`.** Вся численность, активная или только прошедшие оценку? | Возможна одна доп. мера | Активная численность — она же стоит в отчёте рядом. Если оценку проходят не все, добавляется мера «оценённых»; структура не меняется. |
| 4 | **Момент отсчёта `time_to_fill`.** От заявки, от публикации или от согласования? Считаем ли отменённые вакансии? | Расчёт в GP | От публикации до выхода человека; отменённые в знаменатель не идут — для этого `fill_days_cnt` и заведён отдельно от `vac_closed_cnt`. |
| 5 | **Перевод против увольнения.** Уход в дочернюю компанию — отток или перевод? | Значение `event_type` | На текучесть влияет напрямую, но решается классификацией в источнике: витрина принимает любой ответ без правок. |
| 6 | **Доступы.** Кто какое поддерево может выбрать в фильтре `unit`? | Слой API | Права по поддереву на уровне API, не на уровне фронта. Витрина к этому готова: `unit_path` — префикс, права проверяются им же. |
| 7 | **Реидентификация.** В команде из 12 человек комбинация «женщина, 45+, грейд 5» — это конкретный человек. | Слой API | Внутри своего поддерева руководитель эти данные и так знает. За его пределами — либо запрет выбора узла (п. 6), либо подавление ячеек меньше k. Рекомендация: сначала п. 6, подавление — по требованию ИБ. |
| 8 | **Источник этапов воронки найма.** Сейчас коэффициенты выдуманы. | Доп. меры в `f_vacancy_month` | Пока источника нет — вкладку «Воронка» не переводить на прод-данные и оставить сноску. |

Вопрос про историю оргструктуры здесь был и снят: **история приходит готовой из
источника**, восстанавливать и пересчитывать её на стороне отчёта не нужно.
...
1. **Сначала то, без чего нельзя проверить остальное.** Численность — знаменатель
   ...
   текучесть. Проработка конкретных метрик идёт параллельно и ничего не задерживает:
   зерно витрин от неё не зависит.
...
```mermaid
flowchart LR
  E0["Э0<br/>Инвентаризация<br/>источников"] -. параллельно .-> E1
  E1["Э1<br/>Дерево + численность"] --> E2["Э2<br/>Разрезы состава"]
...
### Э0 · Инвентаризация источников (идёт параллельно, не блокирует)

**Зачем.** Собрать то, что нужно знать про источники, — но **не ждать**, пока
определения метрик утрясутся. Каркас витрин от них не зависит (см. «Это каркас, а не
финальное ТЗ» в начале документа), поэтому Э0 и Э1 идут одновременно.

**Что делаем.**

- Инвентаризация систем: доступ, глубина истории, задержка поставки, кто владелец.
- Паспорта метрик заводим **как живой документ, а не как ворота**: колонки
  фиксированы (определение, источник, числитель, знаменатель, правило агрегации,
  владелец), строки заполняются по мере проработки. Заполненный по макету черновик —
  в §7.
- Находим **эталонный отчёт**, с которым будем сверять численность. Вот это
  действительно нужно к концу Э1 — без него у Э1 нет критерия приёмки.

**Результат.** Карта источников с ответственными + эталон для сверки численности.

**Что здесь важно не сделать.** Не превращать этап в согласование девятнадцати
определений до первой строчки кода. Определение метрики меняет выражение внутри
`SELECT`; за всё время проработки не должно появиться уточнения, которое меняет зерно
витрины. Если такое уточнение появилось — это сигнал, что зерно выбрано неверно, и
разбираться надо с каркасом, а не с расписанием.
